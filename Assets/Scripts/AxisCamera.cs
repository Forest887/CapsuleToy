...
{//X軸の角度を制限するための変数
    ...
    [SerializeField] bool inversionX;
    [SerializeField] bool inversionY;

    //Cameraが回転するスピード
    [SerializeField] float rotate_speedX = 3;
    [SerializeField] float rotate_speedY = 3;
    //Axisの位置を指定する変数
    ...
    void Update()
    {
        ...
        //Cameraの角度にマウスからとった値を入れる 反転可能
        if (inversionX && inversionY)
        {
            transform.eulerAngles += new Vector3(
                Input.GetAxis("Mouse Y") * rotate_speedY * -1,
                Input.GetAxis("Mouse X") * rotate_speedX * -1
                , 0);
        }
        else if (inversionX)
        {
            transform.eulerAngles += new Vector3(
                Input.GetAxis("Mouse Y") * rotate_speedY,
                Input.GetAxis("Mouse X") * rotate_speedX * -1
                , 0);
        }
        else if (inversionY)
        {
            transform.eulerAngles += new Vector3(
                Input.GetAxis("Mouse Y") * rotate_speedY * -1,
                Input.GetAxis("Mouse X") * rotate_speedX
                , 0);
        }
        else
        {
